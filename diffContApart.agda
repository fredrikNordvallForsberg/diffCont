...
module _ {l}{X : Set l} where
  ...
  subst : forall {k x y} -> (P : X -> Set k) -> x ~ y -> P x -> P y
  subst P r~ p = p
...
inl-injective : {A B : Set} -> {x y : A} ->
                _~_ {X = A + B} (ff , x) (ff , y) -> x ~ y
inl-injective r~ = r~
...
record Fam (D : Set) : Set1 where
  ...
  field
    indexSet : Datoid
    el : Datoid.Data indexSet -> D

  index : Set
  index = Datoid.Data indexSet

  eq? : (x y : index) -> Dec (x ~ y)
  eq? = Datoid._~?_ indexSet
open Fam
